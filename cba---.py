...
nama_pdf = f"struk_{data.plat}.pdf"

doc = SimpleDocTemplate(nama_pdf)
styles = getSampleStyleSheet()

isi_pdf = [
    Paragraph("SMART PARKING SYSTEM", styles["Title"]),
    Spacer(1, 20),

    Paragraph(f"Plat : {data.plat}", styles["BodyText"]),
    Paragraph(f"Jenis : {data.jenis}", styles["BodyText"]),

    Paragraph(
        f"Jam Masuk : {data.jam_masuk.strftime('%d-%m-%Y %H:%M:%S')}",
        styles["BodyText"]
    ),

    Paragraph(
        f"Jam Keluar : {jam_keluar.strftime('%d-%m-%Y %H:%M:%S')}",
        styles["BodyText"]
    ),

    Paragraph(f"Lama Parkir : {lama_parkir} Jam", styles["BodyText"]),
    Paragraph(f"Metode Pembayaran : {metode}", styles["BodyText"]),
    Paragraph(f"Total Bayar : Rp {total_bayar:,}", styles["BodyText"])
]

doc.build(isi_pdf)
doc.build(isi_pdf)
with open(nama_pdf,"rb") as file:
    st.download_button(
...
# =====================================================
    ...
    st.subheader("🚗 Statistik Jenis Kendaraan")

    aktif = st.session_state.parkir.tampilkan_semua()

    if not MATPLOTLIB_READY:
        st.error("Matplotlib belum terinstall. Grafik tidak bisa ditampilkan.")
    else:
        if len(aktif) > 0:
        ...
        ax2.set_title("Komposisi Kendaraan")
        ax2.axis("equal")  # biar pie chart nggak lonjong
